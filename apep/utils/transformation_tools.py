...
def state_local_to_global(state_local, pose0):
    """
        torch only now.
        state_local: x, y, heading, vx, vy
    """

    zeros = torch.zeros_like(pose0[..., [0]])

    poses = local_to_global(state_local[..., [0, 1, 2]], pose0)
    velocity = local_to_global(state_local[..., [3, 4, 2]], torch.cat([zeros, zeros, pose0[..., [2]]], dim=-1))

    state = state_local.clone()
    state[..., [0, 1, 2]] = poses
    state[..., [3, 4]] = velocity[..., :-1]
    return state
